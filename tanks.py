...
class Connection:
    ...
    def __init__(self, sock=None):
        if sock is None:
            self.sock = websocket.socket.socket(
                            websocket.socket.AF_INET, websocket.socket.SOCK_STREAM)
        else:
            self.sock = sock
    ...
    def receive(self, lenght):
        ...
        while bytes_recd < lenght:
            chunk = self.sock.recv(min(lenght - bytes_recd, 2048))
            if chunk == b'':
                raise RuntimeError("socket connection broken")
            chunks.append(chunk)
...
if __name__ == "__main__":
    ...
    data = s.receive(4)
    myint = websocket.struct.unpack("!i", data)[0]
    data = s.receive(myint)
    strr = websocket.struct.unpack(str(myint) + 's', data)[0]
    tm = Message(strr)
